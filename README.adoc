...
If you're version of Vim supports popups (i.e. version 8.1.1517 or later) the menu
will appear in a popup window. Otherwise it will appear as a "drawer" that slides up
from the bottom of the window. Note that navigation in the slide-up menu is somewhat
limited compared to the popup.

==== Popup

kbd:[J]/kbd:[K] or kbd:[Down]/kbd:[Up] :: Select row below/above the currently
selected row.
...
kbd:[Enter] :: Move the cursor to the section header matching the selected row. The
               ...
               a couple second the popup will be closed.
...
the location of any sections in the saved hierarchy it will rescan the file and show
the updated hierarchy the next time the menu is opened. Nonetheless, you may
